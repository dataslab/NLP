...
def _convert_examples_to_features(examples,label2id, tokenizer,max_seq_length=config.max_seq_length): #labellist是get_label的结果
   ...
   # for (ex_index, example) in tqdm(enumerate(examples), desc="convert examples"): #example(guide、text、label)
    for (ex_index, example) in enumerate(examples): #example(guide、text、label)
...
#预测的时候输入单一句子 text是str类型的
def get_SingleData(ori_text,label2id,tokenizer,max_size=128):
    #把预测中的空格换成其他的符号

    #ori_text=ucd.normalize("NFKC", ori_text).replace(' ', '-') 
    ...
    examples=[]
    text_list=list(ori_text) #先变成list
    text_split=_cut_list(text_list,max_size-2)#长度大于max_size-2的文本就拆成多个文本 [cls],[sep]

    #处理分割文本 长度大的拆分成多个小的文本
    for text in text_split:
        ...
        input_text+=text[-1]
        input_label+="O"
        examples.append(_InputExample(guid=0, text=input_text, label=input_label))
