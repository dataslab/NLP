...
def test(test_loader, test_feature, device, model,id2label,tokenizer):
    ...
    return  result_label,result_text
...
def get_test_str(test_path=config.test_ori_path):
    ...
    for i in range(len(test_data)):
        if i==6073:
            test_data[i]="本人近日误入广发银行的坑，以此教训!!建议大家以后慎入广发银行信用卡!!!!"
...
if __name__ == '__main__':
  ...
  #  model=BertSoftmaxForNer.from_pretrained(config.bert_path,config=bert_config,num_labels=len(label_list))
    ...
    #加载参数
    model.load_state_dict(torch.load(config.model_save_path_final,map_location={'cuda:0':"cuda:2",'cuda:1':"cuda:2"}))
    ...
    for i in tqdm(range(0,len(test_list))):
        ...
        single_label_list,single_text_list=test(test_dataloader, test_features, config.device, model,id2label,tokenizer)
        
        #将single_label_list拼接
        single_label=[]
        for j in range(0,len(single_label_list)):
            single_label.extend(single_label_list[j])
        result_list.append(single_label)

        single_text=[]
        for j in range(0,len(single_text_list)):
            single_text.extend(single_text_list[j])
        text_list.append(single_text)
        


    #输出所有实体
    entity_dic=extraction_entity(text_list,result_list)
    #保存文本
