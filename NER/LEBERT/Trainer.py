...
def train(model, args, train_dataset, dev_dataset, test_dataset, label_vocab, tb_writer, model_path=None):
    ...
    """
    #继续训练的checkpoint
    #model_path="data/result/NER/match/bert_crf_3/checkpoint-1890"
    #如果需要对抗训练
    if args.pgd:
        pgd=PGD(model)
    else:
    ...
    for epoch in train_iterator:
        ...
        if args.evaluate_during_training:
            ...
            if f1_best<metrics['f1']:
                f1_best=metrics['f1']
                output_dir = os.path.join(args.output_dir,"bestf1")
...
def evaluate(model, args, dataset, label_vocab, global_step, description="dev", write_file=False):
    ...
    #写一个直观的result
    temp=args.dev_dataset.split("_")[-1]
    temp_idex=temp.split(".")[0]
    save_result("{}_{}_v2".format(args.model_type,temp_idex),p,r,f1)
...
def main():
    ...
    train_data_file = os.path.join(args.data_dir, args.train_dataset)
    # if only has test_set no dev_set, such as msra NER
    if "msra" in args.data_dir:
        dev_data_file = os.path.join(args.data_dir, args.dev_dataset)
    else:
        dev_data_file = os.path.join(args.data_dir, args.dev_dataset)
    test_data_file = os.path.join(args.data_dir, args.test_dataset)
  #  data_files = [train_data_file, dev_data_file, test_data_file]
    ...
    # return eval_output #预测
    if args.do_predict:
        logger.info("*** Test Evaluate ***")
        print(test_data_file)
        test_dataset = TaskDataset(test_data_file, params=dataset_params, do_shuffle=False)
        global_steps = args.model_name_or_path.split("/")[-2].split("-")[-1]
        eval_output, _ = evaluate(model, args, test_dataset, label_vocab, global_steps, description="predict", write_file=True)
        eval_output["global_steps"] = global_steps
