...
num_filters=256 #卷积核个数 channel_num
filter_sizes=(3,3,3) #卷积核尺寸
...
class BERTRNN(nn.Module):
    ...
    def forward(self, x):   #(x, seq_len, mask)
        ...
        out, _ = self.lstm(encoder_out)  #(output),(h_n,c_n)      tensor的格式(batch , seq , feature)  因为batch_first为true 否则为(seq,batch,feature)
